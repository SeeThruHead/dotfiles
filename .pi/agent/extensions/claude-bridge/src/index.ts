...
function mapToolArgs(
	...
	switch (normalized) {
		...
		case "edit": {
			const editPath = resolvePath(input.file_path ?? input.path);
			if (Array.isArray(input.edits)) {
				return { path: editPath, edits: (input.edits as Array<Record<string, unknown>>).map((e) => ({ oldText: e.old_string ?? e.oldText ?? e.old_text, newText: e.new_string ?? e.newText ?? e.new_text })) };
			}
			return { path: editPath, oldText: input.old_string ?? input.oldText ?? input.old_text, newText: input.new_string ?? input.newText ?? input.new_text };
		}
